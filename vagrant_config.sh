...
function update_system_packages() {
    ...
    if [ -f /etc/os-release ]; then
        . /etc/os-release
        if [ "$NAME" == 'Ubuntu' ]; then
            add-apt-repository universe || install_error "Couldn't enable universe repository"
        fi
    fi
    apt-get update || install_error "Couldn't update package list"
    apt-get -y upgrade || install_error "Couldn't upgrade packages"
}

function install_dependencies() {
    install_log "Installing required packages"
    apt-get install -y screen rsync zip jq || install_error "Couldn't install dependencies"
}
...
install_log "Installing Minecraft Server Manager"
install_msm
chsh -s /bin/bash minecraft

install_log "Installing Spigot BuildTools"
